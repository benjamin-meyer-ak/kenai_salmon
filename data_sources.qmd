...
# WORKING HERE on trying to understand and adjust scaling factors
...
pu_fig2 <- uci_pu2 %>%
  ...
  ggplot() +
  geom_bar(aes(x = as.numeric(year), y = total_count, fill = species), stat = "identity") +
  geom_line(aes(x = as.numeric(year), y = household_days_fished), color = "blue") +
  scale_y_continuous(sec.axis = sec_axis(~ .*5, labels = number_format(scale=1/scale_factor), name="Houeehold Days Fished")) +
  xlab("") +
  ylab("Personal Use Harvest") +
  xlim(comm_year_min,comm_year_max) +
  theme_classic()
